...
function Jobs() {
  ...
  const handleCompanyClick = (country, company) => {
...
//     <div className="flex flex-col items-center justify-center">
    //   <h2 className="text-3xl font-bold mb-4">Job Vacancies</h2>
    //   {vacancies.map((vacancy, index) => (
    //     <div className='group hover:bg-gray-100'>
    //         <div key={index} className="bg-white group-hover:bg-gray-100 rounded-md p-4 mb-4 w-[95vw] sm:w-[80vw] md:w-[70vw]">
    //       <div className='cursor-pointer'>
    //         <h3 className="text-xl font-bold">{vacancy.title}</h3>
    //         <p className="text-gray-600">{vacancy.company}</p> <p className="text-gray-600">{vacancy.location}</p>
    //       </div>
      // <p className="mt-2">{vacancy.description}</p>
      //     <h2 className='font-bold mt-3 text-gray-700'>Qualification</h2>
      //     <p className="">{showMore? vacancy.qualification : vacancy.qualification.slice(0,100)+"..."}</p>
      //     <button className='text-green-600 underline' onClick={(()=>setShowMore(!showMore))}>{showMore? "less" : "more"}</button>
      //     <p className="mt-2 text-blue-800">{vacancy.slots} slots available</p>
      //     <p className="text-gray-600 mt-2">Application Deadline: {vacancy.deadline}</p>
      //     <button className="bg-blue-500 text-white px-4 py-2 mt-4 rounded-md ">Apply Now</button> 
    //     </div>
    //     <hr />
    //     </div>
    //   ))}
